...
Serial.println("Da khoi tao servo cua");
...
void pauseHeavyTasks() {
  ...
  if (result == ESP_OK) {
    ...
    Serial.printf(
      "Tam dung camera that bai, ma loi: 0x%x\n",
      result
    );
  }
}
...
void resumeHeavyTasks() {
  Serial.println("Khoi phuc camera sau khi OTA that bai");

  if (initCamera()) {
    Serial.println("Da khoi phuc camera");
  } else {
    Serial.println("Khoi phuc camera that bai");
  }
}
...
void mqttCallback(
  ...
  // Xử lý lệnh OTA
  if (topicString == OTA_COMMAND_TOPIC) {
    otaHandleMqttMessage(topic, payload, length);
    return;
  }

  // Xử lý các lệnh từ AI, MQTTBox hoặc Node-RED
...
void prepareForDeepSleep() {
  Serial.println("Dang cho MQTT gui het du lieu...");

  // Cho MQTT thời gian đẩy hết dữ liệu ra mạng
  const unsigned long startTime = millis();
  ...
  Serial.println("Da ngat MQTT va WiFi");
}
void openDoor() {
  ...
  Serial.println("CUA SE TU DONG DONG SAU 5 GIAY");
}
...
void handleDoorAutoClose() {
  if (
    doorIsOpen &&
    millis() - doorOpenedTime >= DOOR_AUTO_CLOSE_MS
  ) {
